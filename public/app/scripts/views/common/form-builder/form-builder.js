(function($, undefined){
	...
	****************/
	...
	EditorView.BaseView = function(options) {
		...
		this.getUiDataSource = function() {
			...
			if(this.data.dataSource instanceof Array) {
				...
				request.done(function(resp){
					var resp = self.data.dataSource.afterRequest && self.data.dataSource.afterRequest(resp) || resp;
					deferred.resolve(resp);
				})
			}
			return deferred.promise();
	...
	EditorView.TextView = function (options) {
		...
		this.template_ui = '<label class="input_placeholder">{{placeholder}}</label><input class="all_input form-control" id={{name}} name={{name}} type={{type}}></input>';
	...
	EditorView.SelectView = function(options) {
 		...
 		this.template_ui = [
		'<select name={{name}} id={{name}} class="all_select form-control">',
		'</select>'
		].join("");
	...
	EditorView.TextareaView = function(options){
		...
		this.template_ui = '<label class="input_placeholder">{{placeholder}}</label><textarea class="all_textarea form-control" name={{name}}></textarea>';
	...
	EditorView.NestedSelectView = function(options) {
		...
		this.template_ui_item = '<label class="inline">{{formCaption}}</label><select  class="form-control" name={{name}}></select>';
		this.selector = 'select';
		this.itemTemplate = '<option value={{value}}>{{text}}</option>';
			...
			if(this.selectCompnent) {
				...
				for(var i in selectOrder) {
					...
					var options = $.extend({}, this.options , {
						firstValue : selectOrder[i].firstValue,
		...
		this.bindEvents = function() {
			...
			this.$viewEl.on('change.form', 'select', function(ev){
				...
				var index = findIndexInArrByContent(self.data.selectOrder, {name : handler});
				if(value == self.data.selectOrder[index].firstValue) {
					self.clearAfter(handler);
					return;
				}
				self.changeSelect(handler,value);
			})
		}
		...
		this.changeSelect = function(item, value) {
			var selectOrder = this.data.selectOrder,
				...
				name = selectOrder[index] && selectOrder[index].name,
				obj = {};
			if(!name) {
				return;
			}
			var dataSource = this.data.selectOrder[index].dataSource;
			//
			...
			var request = makeRequest(dataSource.url,  obj, 'POST');
			request.done(function(resp) {
				var data = dataSource.afterRequest && dataSource.afterRequest(resp) || resp;
		...
		this.getData = function() {
			...
			for(var i in this.data.selectOrder) {
				...
				var itemValue = (value == this.data.selectOrder[i].firstValue ? '' : value);
				data.push(itemValue);
			}

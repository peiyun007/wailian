define(['talent'
	...
	return talent.ItemView.extend({
		template : jst['task/task-detail-applicator-single']
		...
		,onRejectUserHandler : function(ev) {
			var self = this;
			var id = talent.$(ev.currentTarget).parents('tr').attr('data-id');
			this.makeRequest('qyneed/refuse_user_from_qyneed', {
				ids_str : [id]
			}).done(function(resp){
				self.trigger('update', resp);
			})
		}
		,onAllowUserHandler : function(ev) {
			var self = this;
			var id = talent.$(ev.currentTarget).parents('tr').attr('data-id');
			this.makeRequest('qyneed/approve_user_to_qyneed', {
				ids_str : [id]
			}).done(function(resp){
				self.trigger('update', resp);
			})
